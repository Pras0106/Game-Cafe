...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    die("ID Komputer tidak valid.");
}
$id = (int) $_GET['id'];

// Ambil data playstation (prepared statement lebih aman)
$stmt = $conn->prepare("
    SELECT id_komputer, harga_per_jam, tipe_pc, status_komputer, url_gambar, Spesifikasi
    FROM komputer 
    WHERE id_komputer = ?
");
...
if (!$ps) {
    die("Komputer tidak ditemukan.");
}

// Ambil aksesoris yang berelasi
$stmt = $conn->prepare("
    SELECT id_aksesoris, id_komputer, nama, url_gambar
    FROM aksesoris_pc
    WHERE id_komputer = ?
...
<head>
    ...
    <title>Detail Komputer #<?= (int)$ps['id_komputer'] ?></title>
...
<body class="bg-white text-gray-900">
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
            ...
            <div class="flex gap-3 mt-3" id="thumbnailContainer">
                ...
                <img class="product-thumbnail thumbnail-item thumbnail-active"
                    ...
                    data-type="ps"
                    data-name="<?= htmlspecialchars($ps['tipe_pc']) ?>"
                    alt="Komputer">
        ...
        <!-- KOLOM INFO -->
        <div>
            <h1 id="title" class="text-3xl font-bold relative inline-block after:content-[''] after:absolute after:left-0 after:-bottom-1 after:w-0 after:h-[3px] after:bg-black after:transition-all after:duration-300 hover:after:w-full"><?= htmlspecialchars($ps['tipe_pc']) ?></h1>

            <br>
            <p id="status_komputer"
                class="mt-2 <?= $ps['status_komputer'] === 'Tersedia'
            ...
            <p class="mt-3 text-gray-700">
                Lokasi : Nomor <?= (int)$ps['id_komputer'] ?>
            </p>

            <?php if (!empty($ps['Spesifikasi'])): ?>
                <div class="mt-4">
                    <h3 class="font-semibold text-lg mb-2">Spesifikasi</h3>
                    <ul class="list-disc list-inside space-y-1 text-gray-700">
                        <?php
                        // Pecah teks berdasarkan newline
                        $specs = preg_split("/\r\n|\n|\r/", $ps['Spesifikasi']);
            ...
            <p class="mt-4 text-xl font-semibold" id="price">Rp <?= number_format($ps['harga_per_jam'], 0, ',', '.') ?> / jam</p>

            <div class="mt-6 flex gap-3">
                <?php
                $bookingLink = isset($_SESSION['id_pelanggan'])
                ? "booking.php?tipe=komputer&id=" . (int)$ps['id_komputer']
                : "login.php?redirect=booking&tipe=komputer&id=" . (int)$ps['id_komputer'];            
                ?>
                ...
                <a href="sewa_pc.php" class="px-4 py-3 border rounded-full">Kembali</a>
            </div>
        ...
        document.querySelectorAll('.thumbnail-item').forEach(th => {
            th.addEventListener('click', function() {
                ...
                if (type === 'aksesoris' && name) {
                    ...
                    // document.getElementById('title').innerText = "<?= addslashes(htmlspecialchars($ps['tipe_pc'])) ?>";
                }
            });
        });
    </script>
